...
val list = 1 to 20;System.out.println("""list  : scala.collection.immutable.Range.Inclusive = """ + $show(list ));$skip(53); 
for (i <- list) {
	if (i % 3 == 0) {
		println(i)
	}
...
val newList = list.filter(_ % 3 == 0);System.out.println("""newList  : scala.collection.immutable.IndexedSeq[Int] = """ + $show(newList ));$skip(28); 
