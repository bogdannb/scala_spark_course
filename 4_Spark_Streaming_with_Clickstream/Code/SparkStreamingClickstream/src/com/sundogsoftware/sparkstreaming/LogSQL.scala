...
object LogSQL {
  ...
  def main(args: Array[String]) {
    ...
    val conf = new SparkConf().setAppName("LogSQL").setMaster("local[*]").set("spark.sql.warehouse.dir", "temp/spark_sql_warehouse")
    ...
    //Exercise - instead of writing data to console write to different formats:
    //-JSON
    //-CSV
    //-Parquet

    // Kick it off
    ssc.checkpoint("temp/checkpoint_log_sql/")
    ssc.start()
