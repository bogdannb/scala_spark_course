...
object StructuredStreaming {
   ...
   def main(args: Array[String]) {
      ...
      val spark = SparkSession
        ...
        .master("local[*]")
        .config("spark.sql.warehouse.dir", "temp/spark_sql_warehouse_streaming") // Necessary to work around a Windows bug in Spark 2.0.0; omit if you're not on Windows.
        .config("spark.sql.streaming.checkpointLocation", "temp/checkpoint_structure_streaming")
        .getOrCreate()
