...
object WordCount {
    ...
    def main(args: Array[String]) {
        ...
        val selectedLines = lines.filter(line => line.toLowerCase.contains("basics"))
        selectedLines.foreach(line => println(line))
        //EXERCISE - count all the lines that have the word "Basics" (ignoring case) into them
        println(s"Number of lines ${selectedLines.count()}")
