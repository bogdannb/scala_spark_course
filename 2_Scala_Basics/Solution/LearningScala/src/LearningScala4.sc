...
//shipList.map(ship => ship.reverse)
...
val sum2 = numberList.reduce(_ + _)
println(sum)
println(sum2)
...
val newList = list.filter(i => i % 3 == 0)
val newList2 = list.filter(_ % 3 == 0)
newList.foreach(i => println(i))
...
val count = words.map(_.toLowerCase).groupBy(identity).mapValues(_.size)
